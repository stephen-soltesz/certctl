...
import (
	...
	"github.com/stephen-soltesz/certctl/certfile"
)

var helpMessage = `NAME:
  certctl - create TLS CA and certs for servers and clients.

DESCRIPTION:
  Systems that require TLS for security but do not require use of the public PKI
  infrastructure can easily generate their own CA using certctl. Your CA can
  then be used to issue intermediate, server, and client certificates.

  For better security, the root CA should be kept offline and intermediate
  certificates used to issue server and client certs.

EXAMPLES:

  certctl ca --hostname ca.example.com

    ca generates a CA, as well as two intermediate certificates for server and
	client certificates.

  certctl server --hostname server.example.com

	server uses the server intermediate certificate and issues a new server
	key and certificate.

  certctl client --hostname client1.example.com

	client uses the client intermediate certificate and issues a new client
	key and certificate.
...
var (
	// TODO: support sub commands using cobra or similar go packages.
	// cert creation.
...
func selfSignCert(c *x509.Certificate, certFile, keyFile string) {
	...
	certfile.WriteCertFile(derBytesCert, certFile)
	certfile.WriteRSAKeyFile(newPrivKey, keyFile)
}

func signCert(c *x509.Certificate, certFile, keyFile, signerCertFile, signerKeyFile string) {
	signerCert, err := certfile.ReadCertFile(signerCertFile)
	if err != nil {
		log.Fatalf("Failed to read signer certificate: %s", err)
	}

	signerKey, err := certfile.ReadRSAKeyFile(signerKeyFile)
	if err != nil {
